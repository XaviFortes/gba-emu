...
fn boot_system(gba: &mut Gba, args: &CliArgs) -> Result<(), ExitCode> {
    let bios_provided = args.bios_path.is_some();

    if let Some(bios) = &args.bios_path {
    ...
    println!("[boot] loading ROM: {}", args.rom_path);
    if let Err(err) = gba.load_rom(&args.rom_path) {
...
pub fn run(args: CliArgs) -> ExitCode {
    ...
    let debug = if args.frames.is_none() && args.bios_path.is_some() && debug.bios_watchdog_frames.is_none() {
        DebugOptions {
